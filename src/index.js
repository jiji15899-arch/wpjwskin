...
.info-card { display: block; border: 1px solid #e2e8f0; border-radius: 15px; padding: 20px; text-decoration: none; color: inherit; background: white; transition: 0.2s; }
.info-card:hover { border-color: #1a73e8; }
...
// [2] 레이아웃 엔진 (구조 단순화)
function getLayout(title, body) {
  return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${title}</title><style>${CSS_STYLE}</style></head><body><header id="header"><div class="container"><span class="logo-text">정부지원금 알리미</span></div></header>${body}<footer class="footer"><p>© ${new Date().getFullYear()} 정부지원금 알리미</p></footer></body></html>`;
}
...
app.get('/', async (c) => {
  try {
    ...
    contentHtml += '<div class="hero-section"><span class="hero-urgent">🔥 신청마감 D-3일</span><h2 class="hero-title">나의 <span style="color:#1a73e8">숨은 지원금</span> 찾기</h2><p style="color:#64748b; margin-bottom:15px;">1인 평균 127만원 수령</p><a href="#" class="hero-cta">내 지원금 확인하기 →</a></div>';
    contentHtml += '<div class="info-card-grid">';
    
    if (results && results.length > 0) {
      results.forEach(p => {
        const d = JSON.parse(p.card_data || '{}');
        contentHtml += `<a class="info-card" href="/post/${p.id}"><span class="card-badge">공고중</span><h3 style="font-weight:bold;">${p.title}</h3><div class="card-amount">${d.amount || '상세확인'}</div><div class="card-btn">신청방법 보기 →</div></a>`;
      });
    } else {
      contentHtml += '<p style="text-align:center;">게시글이 없습니다.</p>';
    }
    
    contentHtml += '</div></div>';
    return c.html(getLayout('정부지원금 알리미', contentHtml));
  } catch (e) {
    return c.text("Error: " + e.message);
  }
});

// [4] 상세 페이지 (에러 지점 완전 재작성)
app.get('/post/:id', async (c) => {
  try {
    const postId = c.req.param('id');
    const postData = await c.env.DB.prepare("SELECT * FROM posts WHERE id = ?").bind(postId).first();
    
    if (!postData) return c.text('Not Found', 404);
    
    const adCode = await c.env.DB.prepare("SELECT value FROM settings WHERE key='pl_ads_code'").first('value') || '광고 영역';
    const cardInfo = JSON.parse(postData.card_data || '{}');
    const formattedContent = postData.content.replace(//g, `<div class="ad-slot">${adCode}</div>`);
    
    let detailHtml = '<div class="container">';
    detailHtml += `<h1 style="font-size:24px; font-weight:800; margin:20px 0;">${postData.title}</h1>`;
    detailHtml += '<div class="hero-section" style="text-align:left; padding:20px;">';
    detailHtml += '<div style="color:#64748b; font-size:14px;">지원 금액</div>';
    detailHtml += `<div style="font-size:24px; font-weight:800; color:#1a73e8;">${cardInfo.amount || '상세내용 참고'}</div>`;
    detailHtml += '</div>';
    detailHtml += `<div class="post-content">${formattedContent}</div>`;
    detailHtml += '</div>';
    
    return c.html(getLayout(postData.title, detailHtml));
  } catch (err) {
    return c.text("Error: " + err.message);
  }
});

// [5] 관리자 및 AI 기능 (최대한 단순하게)
app.get('/admin-login', (c) => {
  return c.html('<div style="padding:100px 20px; text-align:center;"><form method="post" action="/api/login"><input type="password" name="pw" placeholder="비밀번호" style="padding:12px; border:1px solid #ddd; border-radius:8px;"><button type="submit" style="padding:12px 20px; background:#1a73e8; color:white; border:none; border-radius:8px; margin-left:5px;">로그인</button></form></div>');
});

app.post('/api/login', async (c) => {
  const body = await c.req.parseBody();
  const dbPass = await c.env.DB.prepare("SELECT value FROM settings WHERE key='admin_password'").first('value');
  if (body.pw === dbPass) return c.redirect('/admin-post');
  return c.text('비밀번호 불일치');
});

app.get('/admin-post', (c) => {
  const adminHtml = '<div class="container"><h2>AI 자동 포스팅</h2><input type="text" id="topic" placeholder="주제" style="width:100%; padding:12px; margin:10px 0;"><button id="btn" onclick="runAI()" style="width:100%; padding:15px; background:#1a73e8; color:white; border:none; border-radius:8px; font-weight:bold;">AI 글 생성</button><div id="res" style="display:none; margin-top:20px;"><input type="text" id="t" style="width:100%; padding:12px; margin-bottom:10px;"><div id="ed" contenteditable="true" style="min-height:400px; border:1px solid #ddd; padding:20px; background:white; border-radius:8px;"></div><button onclick="save()" style="width:100%; padding:15px; background:black; color:white; border:none; border-radius:8px; margin-top:10px;">최종 발행</button></div></div><script>async function runAI(){const b=document.getElementById("btn");b.innerText="작성중...";const r=await fetch("/api/generate",{method:"POST",body:JSON.stringify({topic:document.getElementById("topic").value})});const d=await r.json();document.getElementById("t").value=d.title;document.getElementById("ed").innerHTML=d.content;window.c=d.card;document.getElementById("res").style.display="block";b.innerText="생성완료";}async function save(){await fetch("/api/save-post",{method:"POST",body:JSON.stringify({title:document.getElementById("t").value,content:document.getElementById("ed").innerHTML,card_data:JSON.stringify(window.c||{})})});alert("발행완료");location.href="/";}</script>';
  return c.html(getLayout('AI 포스팅', adminHtml));
});

app.post('/api/generate', async (c) => {
  const { topic } = await c.req.json();
  const prompt = `주제: ${topic}. 정부지원정책 블로그 글 작성. 반드시 JSON으로만 답해: {"title":"제목","card":{"amount":"최대 00만원"},"content":"본문 HTML(h2, p)"}`;
  const aiRes = await c.env.AI.run('@cf/meta/llama-3-8b-instruct', { messages: [{ role: 'user', content: prompt }] });
